@extends('layouts.app')
@section('title', $yazi->baslik)
@section('meta_desc', $yazi->ozet ?? $yazi->baslik)
...
@endphp
{!! json_encode($blogSchema, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) !!}
</script>
@endpush
...
<div class="blog-hero-bg border-b">
    ...
    <div class="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
        <nav class="flex items-center gap-2 text-sm text-dark-400 mb-6">
            <a href="{{ route('blog') }}" class="hover:text-gold-600 transition"><i class="fas fa-arrow-left mr-1"></i>Blog</a>
            <i class="fas fa-chevron-right text-[10px] text-gray-300"></i>
            <span class="text-dark-900 font-medium truncate">{{ $yazi->baslik }}</span>
        </nav>
        <div class="flex items-center gap-4 mb-5">
            ...
            <span class="text-sm text-dark-400"><i class="far fa-calendar text-gold-500 mr-1"></i>{{ $yazi->created_at->format('d.m.Y') }}</span>
            <span class="text-sm text-dark-400"><i class="far fa-eye text-gold-500 mr-1"></i>{{ number_format($yazi->goruntulenme) }} okunma</span>
        </div>
        <h1 class="text-3xl md:text-5xl blog-title">{{ $yazi->baslik }}</h1>
        @if($yazi->ozet)
            <p class="mt-4 text-lg text-dark-500 leading-relaxed max-w-2xl">{{ $yazi->ozet }}</p>
        @endif
    </div>
</div>
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    <div class="rounded-xl overflow-hidden mb-10 shadow-lg">
        ...
        @else
            <img src="https://picsum.photos/seed/{{ $yazi->id }}/1200/600"
                 class="w-full h-auto object-cover"
                 loading="eager"
                 style="max-height: 500px;">
        @endif
    ...
    <div class="mt-16 pt-10 border-t border-dark-100">
        <h2 class="text-2xl font-serif font-bold text-dark-900 mb-8">Yorumlar ({{ $yorumlar->count() }})</h2>
                    ...
                    <div class="bg-white rounded-xl border border-dark-100 p-6">
                        <div class="flex items-start gap-4">
                            <div class="w-10 h-10 rounded-full bg-gradient-to-br from-gold-400 to-gold-500 flex items-center justify-center text-white text-sm font-bold shrink-0">
                                {{ substr($yorum->ad_soyad, 0, 1) }}
                            </div>
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center justify-between gap-2 mb-1">
                                    <span class="font-semibold text-dark-900 text-sm">{{ $yorum->ad_soyad }}</span>
                                    <span class="text-xs text-dark-400">{{ $yorum->created_at->diffForHumans() }}</span>
                                </div>
                                <p class="text-sm text-dark-600 leading-relaxed">{{ $yorum->yorum }}</p>
                            </div>
                        </div>
                    </div>
                @endforeach
        ...
        <div class="bg-white rounded-xl border border-dark-100 p-6 md:p-8">
            ...
            <form action="{{ route('blog.yorum', $yazi->slug) }}" method="POST">
                @csrf
                <div class="grid md:grid-cols-2 gap-4 mb-4">
                    <div>
                        <label class="block text-sm font-medium text-dark-700 mb-1.5">Ad Soyad <span class="text-red-400">*</span></label>
                        <input type="text" name="ad_soyad" value="{{ old('ad_soyad', auth()->user()->ad_soyad ?? '') }}" required
                ...
                <div class="mb-4">
                    ...
                    <textarea name="yorum" rows="4" required
                              class="w-full px-4 py-2.5 border border-dark-100 rounded-lg text-sm focus:border-gold-400 focus:ring-2 focus:ring-gold-500/10 transition resize-y">{{ old('yorum') }}</textarea>
                </div>
    ...
    <div class="mt-12 pt-8 border-t border-dark-100 flex items-center justify-between">
        <a href="{{ route('blog') }}" class="btn-outline-gold !py-2.5"><i class="fas fa-arrow-left mr-1"></i>Tüm Yazılar</a>
        <a href="{{ route('products') }}" class="text-sm text-dark-400 hover:text-gold-600 transition">Alışverişe Devam Et <i class="fas fa-arrow-right ml-1"></i></a>
    </div>
</div>
@endsection