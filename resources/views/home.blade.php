...
@section('meta_desc', 'Kişiye özel ürünlerin buluşma noktası – Her Ürün Size Özel. Size özel tasarlanmış hediyelik eşya, aksesuar ve dekoratif ürünler.')
...
<section class="relative min-h-screen flex flex-col overflow-hidden bg-dark-950 noise-overlay">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 md:py-32 w-full">
            ...
            <div class="reveal">
                ...
                <div class="flex flex-wrap gap-4">
                    <a href="{{ route('products') }}" class="btn-gold"><i class="fas fa-arrow-right"></i> Ürünleri Keşfet</a>
                    <a href="{{ route('satici-basvuru') }}" class="btn-outline-gold"><i class="fas fa-store"></i> Mağazanı Aç</a>
                </div>

                <div class="flex items-center gap-10 mt-14 pt-10 border-t border-dark-800">
                    <div>
                        <p class="text-3xl md:text-4xl font-serif font-bold text-white counter-value" data-target="{{ $toplamUrun }}">0</p>
                        <p class="text-xs text-dark-500 mt-1 tracking-wider uppercase font-sans">Ürün</p>
                    </div>
                    <div class="w-px h-10 bg-dark-800"></div>
                    <div>
                        <p class="text-3xl md:text-4xl font-serif font-bold text-white counter-value" data-target="{{ $toplamMagaza }}">0</p>
                        <p class="text-xs text-dark-500 mt-1 tracking-wider uppercase font-sans">Mağaza</p>
                    </div>
                    <div class="w-px h-10 bg-dark-800"></div>
                    <div>
                        <p class="text-3xl md:text-4xl font-serif font-bold text-white counter-value" data-target="{{ $toplamMusteri }}">0</p>
    ...
    {{-- Hero Banner Slider --}}
    @if(isset($heroBannerlar) && $heroBannerlar->count() > 0)
    <div class="w-full" x-data="{ current: 0, anim: false, sliderTimer: null }" x-init="setTimeout(() => anim = true, 50); sliderTimer = setInterval(() => { current = current + 1 >= {{ $heroBannerlar->count() }} ? 0 : current + 1 }, 5000); $watch('current', () => { anim = false; $nextTick(() => anim = true) })">
        @foreach($heroBannerlar as $i => $banner)
            <div x-show="current === {{ $i }}" x-transition:enter="transition-all duration-500" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" style="{{ $i > 0 ? 'display:none' : '' }}">
                <div class="relative h-[300px] md:h-[450px] flex items-center">
                    <img src="{{ asset('storage/banner/' . $banner->foto) }}" class="absolute inset-0 w-full h-full object-cover" alt="{{ $banner->baslik }}">
                    <div class="absolute inset-0 bg-gradient-to-r from-dark-950/80 via-dark-950/40 to-transparent"></div>
                    <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
                        <div class="max-w-xl">
                            <h2 class="text-3xl md:text-5xl font-serif font-bold text-white mb-3 transition-all duration-700 ease-out" :class="anim ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'" style="transition-delay:0.1s">{{ $banner->baslik }}</h2>
                            @if($banner->alt_baslik)<p class="text-lg text-gold-300 font-serif italic mb-6 transition-all duration-700 ease-out" :class="anim ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'" style="transition-delay:0.3s">{{ $banner->alt_baslik }}</p>@endif
                            @if($banner->link)<a href="{{ $banner->link }}" class="btn-gold text-sm transition-all duration-700 ease-out" :class="anim ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'" style="transition-delay:0.5s">İncele <i class="fas fa-arrow-right ml-1"></i></a>@endif
                        </div>
                    </div>
                </div>
            </div>
        @endforeach
        @if($heroBannerlar->count() > 1)
        <div class="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-20">
            @foreach($heroBannerlar as $i => $banner)
                <button @click="clearInterval(sliderTimer); current = {{ $i }}; sliderTimer = setInterval(() => { current = current + 1 >= {{ $heroBannerlar->count() }} ? 0 : current + 1 }, 5000)" class="w-2.5 h-2.5 rounded-full transition-all" :class="current === {{ $i }} ? 'bg-gold-500 w-8' : 'bg-white/40 hover:bg-white/70'"></button>
            @endforeach
        </div>
        @endif
    </div>
...
<section class="py-20 md:py-28 relative overflow-hidden" style="background:linear-gradient(180deg,#0f0c1a 0%,#0c0a14 30%,#0a0812 60%,#0f0c1a 100%)">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-5 md:gap-6 relative z-20">
            ...
            @endphp
            <div class="group relative reveal" style="transition-delay: {{ $loop->index * 0.08 }}s">
                <a href="{{ route('category', $kategori->slug) }}"
                   ...
                   style="background-color: {{ $c['bg'] }};">
                    ...
                    {{-- Yatay kurdele --}}
                    <div class="absolute top-[1px] left-0 right-0 h-4 z-20 pointer-events-none rounded-b-lg shadow-sm" style="background-color: {{ $c['accent'] }};"></div>

                    {{-- Dikey kurdele --}}
                    <div class="absolute top-0 left-4 w-5 h-full z-20 pointer-events-none group-hover:opacity-80 transition-opacity duration-300" style="background-color: {{ $c['accent'] }};"></div>

                    {{-- İçerik --}}
                    <div class="relative z-10 mt-6">
                        <div class="mx-auto flex items-center justify-center mb-4">
                            @if($kategori->ikon)
                                <i class="{{ $kategori->ikon }} text-3xl text-white group-hover:text-gold-200 group-hover:scale-110 transition-all duration-500"></i>
                            @else
                                <i class="fas fa-gift text-3xl text-white"></i>
                            @endif
                        </div>
                        <h3 class="font-serif font-semibold text-white group-hover:text-gold-200 transition-colors duration-400 text-xl md:text-2xl">{{ $kategori->kategori_adi }}</h3>
                        ...
                        <span class="inline-block text-sm text-gold-300 font-sans tracking-wide bg-gold-500/15 px-3 py-1 rounded-full group-hover:bg-gold-500/30 group-hover:text-gold-200 transition-all duration-400">{{ $kategori->urunler_count ?? 0 }} ürün</span>
                    </div>
                </a>
...
<section class="py-20 md:py-28 bg-cream-100/30">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16 reveal">
            ...
            <a href="{{ route('products') }}" class="btn-outline-gold shrink-0">Tümünü Gör <i class="fas fa-arrow-right ml-1"></i></a>
        </div>
...
{{-- Between Banners --}}
@if(isset($betweenBannerlar) && $betweenBannerlar->count() > 0)
<section class="bg-cream-100/50 border-y border-gold-100/30 py-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid md:grid-cols-{{ min($betweenBannerlar->count(), 3) }} gap-5">
            @foreach($betweenBannerlar as $banner)
                <a href="{{ $banner->link ?: '#' }}" class="group relative overflow-hidden rounded-xl h-48 block {{ $banner->link ? 'cursor-pointer' : 'cursor-default' }}">
                    ...
                    <div class="absolute bottom-4 left-4 right-4">
                        <h3 class="text-white font-serif font-bold text-lg">{{ $banner->baslik }}</h3>
                        @if($banner->alt_baslik)<p class="text-gold-300 text-sm">{{ $banner->alt_baslik }}</p>@endif
                    </div>
                </a>
            @endforeach
        </div>
    ...
    <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-5 reveal">
            <div class="text-center p-8 rounded-xl bg-white/[0.02] border border-white/[0.06] stat-glow">
                <p class="text-4xl md:text-5xl font-serif font-bold text-white counter-value" data-target="{{ $toplamUrun }}">0</p>
            ...
            <div class="text-center p-8 rounded-xl bg-white/[0.02] border border-white/[0.06] stat-glow">
                <p class="text-4xl md:text-5xl font-serif font-bold text-white counter-value" data-target="{{ $toplamMagaza }}">0</p>
                <div class="w-8 h-0.5 bg-gold-500/60 mx-auto my-3"></div>
                <p class="text-sm text-dark-400 font-sans tracking-wider uppercase">Mağaza</p>
            </div>
            <div class="text-center p-8 rounded-xl bg-white/[0.02] border border-white/[0.06] stat-glow">
                <p class="text-4xl md:text-5xl font-serif font-bold text-white counter-value" data-target="{{ $toplamMusteri }}">0</p>
                <div class="w-8 h-0.5 bg-gold-500/60 mx-auto my-3"></div>
                <p class="text-sm text-dark-400 font-sans tracking-wider uppercase">Müşteri</p>
            </div>
            <div class="text-center p-8 rounded-xl bg-white/[0.02] border border-white/[0.06] stat-glow">
                <p class="text-4xl md:text-5xl font-serif font-bold text-white"><span class="counter-value" data-target="{{ $memnuniyet }}">0</span>%</p>
                <div class="w-8 h-0.5 bg-gold-500/60 mx-auto my-3"></div>
...
<section class="py-20 md:py-28 bg-cream-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16 reveal">
            ...
            <a href="{{ route('magazalar') }}" class="btn-outline-gold shrink-0">Tüm Mağazalar <i class="fas fa-arrow-right ml-1"></i></a>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5 md:gap-6">
            @foreach($magazalar as $magaza)
                <a href="{{ route('magaza.detail', $magaza->slug) }}" class="group relative bg-white rounded-2xl overflow-hidden card-shine block transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-gold-500/20 reveal">
                    <div class="relative h-28 overflow-hidden bg-gradient-to-br from-gold-200 to-cream-300">
                         @if($magaza->banner)
                             <img src="{{ \App\Services\ImageService::getUrl($magaza->banner, 400, 200) }}"
                                  ...
                                  height="200">
                         @else
                             <img src="{{ \App\Services\MagazaGorselService::getBannerUrl($magaza->magaza_adi, $magaza->slogan, 400, 200) }}"
                                  class="w-full h-full object-cover transition-all duration-700 group-hover:scale-110"
                                  alt=""
                                  loading="lazy"
                                  width="400"
                        ...
                        <div class="absolute -bottom-8 left-1/2 -translate-x-1/2 z-10">
                            <div class="w-16 h-16 rounded-2xl overflow-hidden border-[3px] border-white shadow-lg shadow-gold-500/10 bg-gradient-to-br from-gold-50 to-cream-100 transition-all duration-500 group-hover:shadow-xl group-hover:shadow-gold-500/20 group-hover:border-gold-200 group-hover:-translate-y-1">
                                 @if($magaza->logo)
                                     <img src="{{ \App\Services\ImageService::getUrl($magaza->logo, 80, 80) }}"
                                          ...
                                          height="80">
                                 @else
                                     <img src="{{ \App\Services\MagazaGorselService::getLogoUrl($magaza->magaza_adi, 80) }}"
                                          class="w-full h-full object-cover"
                                          alt="{{ $magaza->magaza_adi }}"
                                          loading="lazy"
                                          width="80"
                    ...
                    <div class="pt-10 pb-4 px-4 text-center">
                        <h3 class="font-serif font-semibold text-dark-900 text-sm group-hover:text-gold-600 transition-colors">{{ $magaza->magaza_adi }}</h3>
                        @if($magaza->slogan)
                            <p class="text-xs text-dark-600 mt-1 font-medium font-sans line-clamp-1">"{{ $magaza->slogan }}"</p>
                        @else
                            <p class="text-xs text-dark-600 mt-1 font-medium font-sans line-clamp-1">"Biz ki&#351;iye &#246;zel &#252;r&#252;nler tasarlan&#305;yoruz"</p>
                        @endif
                        @if($magaza->sehir)
                            <p class="text-[11px] text-dark-400 mt-2"><i class="fas fa-map-marker-alt text-gold-400 mr-1"></i>{{ $magaza->sehir }}</p>
                        @endif
                    </div>
...
<section class="relative py-28 overflow-hidden bg-dark-950 noise-overlay">
    ...
    <div class="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center reveal">
        ...
        <div class="flex flex-wrap justify-center gap-4">
            <a href="{{ route('satici-basvuru') }}" class="btn-gold"><i class="fas fa-store"></i> Hemen Başvur</a>
            <a href="{{ route('magazalar') }}" class="btn-outline-gold">Mağazaları Keşfet</a>
        </div>
    </div>
</section>
