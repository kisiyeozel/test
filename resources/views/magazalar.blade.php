...
@section('meta_desc', 'En özel ürünler, en özel mağazalardan – Kisiyeozel.org mağazalarını keşfedin')
        ...
        @if($magazalar->count() > 0)
            <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-12 reveal">
                ...
                <p class="text-dark-400 text-sm shrink-0 bg-white/80 backdrop-blur-sm px-5 py-2 rounded-full border border-dark-100/50 shadow-sm flex items-center gap-2">
                    ...
                    <span><strong class="text-dark-800">{{ $magazalar->total() }}</strong> mağaza</span>
                </p>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 md:gap-8">
                @foreach($magazalar as $magaza)
                    <a href="{{ route('magaza.detail', $magaza->slug) }}" class="group relative bg-white rounded-2xl overflow-hidden card-shine block transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-gold-500/20 reveal-scale" style="transition-delay: {{ $loop->index * 0.06 }}s">
                        <div class="relative h-48 md:h-56 overflow-hidden bg-gradient-to-br from-gold-200 to-cream-300">
                            @if($magaza->banner)
                                <img src="{{ \App\Services\ImageService::getUrl($magaza->banner, 600, 300) }}"
                                     srcset="{{ \App\Services\ImageService::getSrcset($magaza->banner, [400, 600, 900]) }}"
                                     ...
                                     height="300">
                            @else
                                <img src="{{ \App\Services\MagazaGorselService::getBannerUrl($magaza->magaza_adi, $magaza->slogan, 600, 300) }}"
                                     class="w-full h-full object-cover transition-all duration-700 group-hover:scale-110"
                                     alt=""
                                     loading="lazy"
                                     width="600"
                            ...
                            <div class="absolute -bottom-11 left-1/2 -translate-x-1/2 z-10">
                                <div class="w-24 h-24 md:w-28 md:h-28 rounded-2xl overflow-hidden border-[5px] border-white shadow-xl shadow-gold-500/15 bg-gradient-to-br from-gold-50 to-cream-100 transition-all duration-500 group-hover:shadow-2xl group-hover:shadow-gold-500/30 group-hover:border-gold-300 group-hover:-translate-y-2">
                                    @if($magaza->logo)
                                        <img src="{{ \App\Services\ImageService::getUrl($magaza->logo, 120, 120) }}"
                                             ...
                                             height="120">
                                    @else
                                        <img src="{{ \App\Services\MagazaGorselService::getLogoUrl($magaza->magaza_adi, 120) }}"
                                             class="w-full h-full object-cover"
                                             alt="{{ $magaza->magaza_adi }}"
                                             loading="lazy"
                                             width="120"
                        ...
                        <div class="pt-16 pb-6 px-5 md:px-6 text-center">
                            <h3 class="font-serif font-bold text-dark-900 text-xl md:text-2xl group-hover:text-gold-600 transition-colors leading-tight">{{ $magaza->magaza_adi }}</h3>
                            @if($magaza->slogan)
                                <p class="text-base text-dark-600 mt-2 font-medium font-sans line-clamp-1">"{{ $magaza->slogan }}"</p>
                            @else
                            ...
                            <div class="flex flex-wrap items-center justify-center gap-x-5 gap-y-2">
                                @if($magaza->sehir)
                                    <span class="flex items-center gap-1.5 text-xs text-dark-400"><i class="fas fa-map-marker-alt text-gold-400"></i>{{ $magaza->sehir }}</span>
                                @endif
                                <span class="flex items-center gap-1.5 text-xs text-dark-400"><i class="fas fa-box text-gold-400"></i>{{ $magaza->urunler_count ?? 0 }} ürün</span>
                            </div>
            ...
            <div class="mt-14 reveal">{{ $magazalar->links() }}</div>
        @else
            <div class="text-center py-24 reveal-scale">
                ...
                <a href="{{ route('satici.magaza') }}" class="btn-gold mt-8 inline-flex"><i class="fas fa-plus mr-2"></i>Mağaza Oluştur</a>
            </div>
        @endif
    </div>
</div>
