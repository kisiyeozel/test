 <?php $__env->startSection('title', 'Banner Yönetimi'); ?>
...
<div class="bg-white rounded-xl border overflow-hidden mb-6" x-data="{ rows: [{id: Date.now()}] }">
    ...
    <div class="p-4">
        <form action="<?php echo e(route('admin.banner-ekle')); ?>" method="POST" enctype="multipart/form-data">
            <?php echo csrf_field(); ?>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                <?php $__empty_1 = true; $__currentLoopData = $bannerlar; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $banner): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3"><img src="<?php echo e(asset('storage/banner/' . $banner->foto)); ?>" class="h-12 w-20 object-cover rounded"></td>
                        <td class="px-4 py-3 font-medium text-gray-800"><?php echo e($banner->baslik); ?></td>
                        <td class="px-4 py-3">
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-700"><?php echo e($banner->pozisyon); ?></span>
                        </td>
                        <td class="px-4 py-3 text-center text-gray-500"><?php echo e($banner->sira); ?></td>
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <button onclick="document.getElementById('edit-<?php echo e($banner->id); ?>').classList.toggle('hidden')" class="text-blue-500 hover:text-blue-600 text-sm"><i class="fas fa-edit"></i></button>
                                <form action="<?php echo e(route('admin.banner-sil', $banner->id)); ?>" method="POST" onsubmit="return confirm('Silmek istediğinize emin misiniz?')">
                                    <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                                    <button class="text-red-500 hover:text-red-600 text-sm"><i class="fas fa-trash"></i></button>
                                </form>
                            </div>
                            <div id="edit-<?php echo e($banner->id); ?>" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onclick="if(event.target===this)this.classList.add('hidden')">
                                <div class="bg-white rounded-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto" onclick="event.stopPropagation()">
                                    ...
                                    <form action="<?php echo e(route('admin.banner-guncelle', $banner->id)); ?>" method="POST" enctype="multipart/form-data">
                                        <?php echo csrf_field(); ?>
                                        <div class="grid md:grid-cols-2 gap-4">
                                            <div class="md:col-span-2"><label class="block text-sm font-medium text-gray-700 mb-1">Başlık</label><input type="text" name="baslik" value="<?php echo e($banner->baslik); ?>" class="w-full border-gray-300 rounded-lg" required></div>
                                            <div class="md:col-span-2"><label class="block text-sm font-medium text-gray-700 mb-1">Alt Başlık</label><input type="text" name="alt_baslik" value="<?php echo e($banner->alt_baslik); ?>" class="w-full border-gray-300 rounded-lg"></div>
                                            <div class="md:col-span-2"><label class="block text-sm font-medium text-gray-700 mb-1">Link</label><input type="text" name="link" value="<?php echo e($banner->link); ?>" class="w-full border-gray-300 rounded-lg"></div>
                                            <div><label class="block text-sm font-medium text-gray-700 mb-1">Pozisyon</label><select name="pozisyon" class="w-full border-gray-300 rounded-lg"><option value="hero" <?php echo e($banner->pozisyon=='hero'?'selected':''); ?>>Hero</option><option value="sidebar" <?php echo e($banner->pozisyon=='sidebar'?'selected':''); ?>>Sidebar</option><option value="between" <?php echo e($banner->pozisyon=='between'?'selected':''); ?>>Arası</option><option value="footer" <?php echo e($banner->pozisyon=='footer'?'selected':''); ?>>Footer</option></select></div>
                                            <div><label class="block text-sm font-medium text-gray-700 mb-1">Sıra</label><input type="number" name="sira" value="<?php echo e($banner->sira); ?>" class="w-24 border-gray-300 rounded-lg"></div>
                                            ...
                                            <div class="md:col-span-2 flex items-center gap-2"><input type="checkbox" name="aktif" value="1" <?php echo e($banner->aktif?'checked':''); ?> class="rounded border-gray-300"><label class="text-sm text-gray-600">Aktif</label></div>
                                        </div>
                                        <div class="flex gap-2 mt-4">
                                            ...
                                            <button type="button" onclick="document.getElementById('edit-<?php echo e($banner->id); ?>').classList.add('hidden')" class="px-4 py-2 text-sm text-gray-600 hover:text-gray-800">İptal</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\yenideneme\resources\views/admin/bannerlar.blade.php ENDPATH**/ ?>